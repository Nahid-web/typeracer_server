...
//connect to DB
const DB =
  "mongodb+srv://nahid:12345@atlascluster.10onywa.mongodb.net/?retryWrites=true&w=majority&appName=AtlasCluster";
  ...
  socket.on("join-game", async ({ nickname, gameId }) => {
    try {
      ...
      if (game.isJoin) {
        ...
        socket.emit(
          "notCorrectGame",
          "The game is progress, please try again later!"
        );
      }
    } catch (error) {
      console.log(error);
...
server.listen(port, "0.0.0.0", () => {
  console.log(`Server stared and running on port ${port}`);
});
